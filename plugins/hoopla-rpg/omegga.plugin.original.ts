 ...
 * 
 * This plugin has been refactored into a modular architecture with clear separation of concerns.
 ...
 * 
 * The main plugin now acts as a coordinator, delegating to these specialized services.
 * This provides better maintainability, testability, and code organization.
 */
...
export default class Plugin implements OmeggaPlugin<Config, Storage> {
  ...
  constructor(omegga: OL, config: PC<Config>, store: PS<Storage>) {
    ...
    this.questService = new QuestService(omegga, store, this.messagingService);
  ...
  async getPlayerData({ id }: PlayerId): Promise<RPGPlayer> {
    return this.playerService.getPlayerData({ id });
  }
  ...
  async setPlayerData({ id }: PlayerId, data: RPGPlayer) {
    return this.playerService.setPlayerData({ id }, data);
  }
  ...
  sendLongMessage(playerId: string, message: string, maxLength: number = 200): void {
    this.messagingService.sendLongMessage(playerId, message, maxLength);
  }
  ...
  getResourceSellPrice(resourceType: string): number {
    return this.resourceService.getResourceSellPrice(resourceType);
  }
  ...
  getBarteringMultiplier(barteringLevel: number): number {
    return this.barteringService.getBarteringMultiplier(barteringLevel);
  }

  // Create a visual progress bar using text characters
  createProgressBar(current: number, total: number, width: number = 20): string {
    return this.progressBarService.createProgressBar(current, total, width);
  }
  ...
  getResourceColor(resourceName: string): string {
    return this.resourceService.getResourceColor(resourceName);
  }
  ...
  async addToInventory({ id }: PlayerId, item: string): Promise<string[]> {
    const player = await this.getPlayerData({ id });
    return this.inventoryService.addToInventory(player, item);
  }

  async removeFromInventory({ id }: PlayerId, item: string): Promise<boolean> {
    const player = await this.getPlayerData({ id });
    return this.inventoryService.removeFromInventory(player, item);
  }
